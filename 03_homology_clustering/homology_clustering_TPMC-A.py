#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# 文件名: homology_clustering_TPMC-A.py
# 描述: 基于物理位置聚类的结果，对 TPMC-A 防御基因进行序列同源性聚类 (CD-HIT)
...
def main():
    print("开始 TPMC-A 防御基因同源聚类分析 (Algorithm: CD-HIT, Threshold: 40%)...")
    
    # === 路径配置 ===
    # 修复了基础路径
    base_dir = "/data6/xiongxinghao/project/anti-virus/liyuanhao/qinzang"
    
    # 依赖上一步(物理聚类)生成的真实坐标 csv 文件
    input_csv = os.path.join(base_dir, "02_physical_clustering/TPMC-A_20260420/real_position_clusters_with_context_TPMC-A.csv")
    
    # 修改为最新的序列所在目录
    genome_dir = os.path.join(base_dir, "02_physical_clustering/TPMC-A_20260420/defense_clusters_sequences")
    
    # 输出目录 (如果是首次运行会自己创建)
    output_dir = os.path.join(base_dir, "03_homology_clustering/TPMC-A_20260420")
...
def extract_sequences(df, genome_dir, output_dir, threads=32):
    print("正在搜寻并提取簇内蛋白质序列...")
    all_proteins_file = os.path.join(output_dir, "TPMC-A_cluster_proteins.faa")
    ...
    # 例如将 'xxx_bin.106.purify_k141_333000' 截断成 'xxx_bin.106.purify'
    base_prefixes = set(df['genome_id'].astype(str).apply(lambda x: x.split('_k141_')[0]).unique())
...
def analyze_clusters(clstr_file, original_df, output_dir):
    ...
    # 保存详细映射表
    detail_path = os.path.join(output_dir, "TPMC-A_homology_clusters_detail.csv")
    ...
    stats_path = os.path.join(output_dir, "TPMC-A_homology_cluster_statistics.csv")
...
def extract_representatives(clusters, all_proteins_file, output_dir):
    ...
    rep_file_path = os.path.join(output_dir, "TPMC-A_homology_cluster_representatives.faa")
