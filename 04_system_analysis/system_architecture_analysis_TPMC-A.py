#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# 文件名: system_architecture_analysis_TPMC-A.py
...
def main():
    print("开始 TPMC-A 新防御系统深度分析与架构挖掘...")
    
    # === 1. 配置路径 ===
    base_dir = "/home/xiongxinghao/data6/project/anti-virus/liyuanhao/qinzang"
    
    # 输入文件 (依赖前面步骤的输出)
    physical_csv = os.path.join(base_dir, "02_physical_clustering/TPMC-A_20260420/real_position_clusters_with_context_TPMC-A.csv")
    homology_detail_csv = os.path.join(base_dir, "03_homology_clustering/TPMC-A_20260420/TPMC-A_homology_clusters_detail.csv")
    homology_stats_csv = os.path.join(base_dir, "03_homology_clustering/TPMC-A_20260420/TPMC-A_homology_cluster_statistics.csv")
    
    # 输出目录
    output_dir = os.path.join(base_dir, "04_system_analysis/TPMC-A_20260420")
    ...
    comp_df.to_csv(os.path.join(output_dir, "TPMC-A_system_architecture_frequency.csv"), index=False)
    ...
    # 生成详细报告
    report_file = os.path.join(output_dir, "TPMC-A_novel_system_candidates_report.txt")
    with open(report_file, "w", encoding='utf-8') as f:
        f.write("=== TPMC-A 高置信度潜在新防御系统候选挖掘报告 ===\n\n")
    ...
    if edge_data:
        pd.DataFrame(edge_data).to_csv(os.path.join(output_dir, "TPMC-A_family_cooccurrence_network.csv"), index=False)
